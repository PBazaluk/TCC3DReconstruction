...
with open(output_file, 'w') as f:
    for i in range(cont):
        ...
        f.write(f'Imagem 1 - Keypoint {i}: Localização (x, y): {kp_img1.pt}\n')
        f.write('\n')
        ...
        f.write(f'Imagem 2 - Keypoint {i}: Localização (x, y): {kp_img2.pt}\n')
        f.write('\n')
    f.write('\n')
    f.write('\n')
...
largura1 = int(img_shadow.shape[1] * 0.5)
altura1 = int(img_shadow.shape[0] * 0.5)
dimensoes1 = (largura1, altura1)
largura2 = int(img.shape[1] * 0.5)
altura2 = int(img.shape[0] * 0.5)
