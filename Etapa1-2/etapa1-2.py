...
def stitch_images(images, output_dir):
    ...
    for i in range(1, len(images)):
        ...
        # Limitar o número de correspondências desenhadas
        show_matches = sorted(good_matches, key=lambda x: x.distance)[:20]

        img_matches = cv.drawMatches(composed_image, kp1, img2, kp2, show_matches, None, 
                                     singlePointColor=None,
                                     matchesMask=None,
                                     flags=cv.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS,
                                     matchesThickness=3)
        cv.imwrite(os.path.join(output_dir, f'matches_{i}.jpg'), img_matches)
...
def main(image_paths, output_path, image_out):
    images = load_images(image_paths)
    if len(images) < 2:
        print("É necessário pelo menos duas imagens para compor.")
        return

    composed_image = stitch_images(images, image_out)
...
if __name__ == "__main__":
    ...
    main(image_files, output_path, image_out)